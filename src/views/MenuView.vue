...
export default {
    ...
    methods: {
      ...
      openImportModal() {
          Swal.mixin({
            customClass: {
              confirmButton: "btn btn-confirm mx-3",
            ...
            if (data.isConfirmed) {
              ...
              fileReader.onload = e => {
                ...
                socket.emit("add menus from file", { data: this.processXLSX(wb.Sheets.Feuil1, startDate) });
              };
...
<style>
.menu-container {
  @apply w-2/3 flex border-4 rounded-3xl shadow-xl shadow-gray-400/30 px-4 pt-6 pb-3 bg-white/60 flex-col items-center relative;
}
.import-part {
  @apply hidden;
}
...
.btn {
  @apply flex w-44 border-4 h-12 justify-center items-center rounded-xl text-black/80 font-semibold border-white bg-white/30;
}
.btn:hover {
  @apply bg-white/30 border-white/70;
}
.btn svg {
...
.btn.disabled {
  @apply border-white/70 bg-white/30 cursor-default;
}
.btn-confirm {
...
.swal2-modal .import-modal-container input[type=date],
...
.general-input .btn-import:hover {
